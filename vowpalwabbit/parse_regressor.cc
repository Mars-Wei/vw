...
void dump_regressor(vw& all, string reg_name, bool as_text)
{
    ...
    save_load_header(all, io_temp, false, as_text);//model header
...
void parse_regressor_args(vw& all, po::variables_map& vm, io_buf& io_temp)
{
    ...
    if (regs.size() > 0)
        io_temp.open_file(regs[0].c_str(), all.stdin_off, io_buf::READ);//open model file

    save_load_header(all, io_temp, true, false);//read, non-text
}
