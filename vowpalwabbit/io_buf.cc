...
{//Return a pointer to the bytes before the terminal.  Must be less than the buffer size.
    //cerr<<"readto_packet()"<<endl;
    assert(i.files.size() == 1);
    pointer = i.space.end;
    //cerr<<"end-begin:"<<i.space.end-i.space.begin<<endl;
    //cerr<<"endloaded-end:"<<i.endloaded-i.space.end<<endl;
    while (pointer != i.endloaded && *pointer != terminal)
        pointer++;
    if (pointer != i.endloaded)
    {//terminal
        ...
        i.space.end = pointer+1;
        pointer -= n;
        //cerr<<"found terminal ok"<<endl;
        return n+1;//with terminal
    }
    else
    {//no terminal
        if (pointer == i.endloaded )
        { // packet processed, reset
            i.space.end = i.endloaded = i.space.begin;//reset
        }
        if( i.fill_packet(i.files[0]) > 0)
            return readto_packet(i, pointer, terminal);
        else//got error
        {
            return 0;
        }
