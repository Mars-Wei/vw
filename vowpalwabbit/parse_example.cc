...
void substring_to_example(vw* all, example* ae, substring example)
{
    ...
    if (tab_location != bar_location){
        ...
        label_space.begin = example.begin;//strip begin tab
    }
...
{
    //cerr<<"call read_features"<<endl;
    vw* all = (vw*)in;
    example* ae = (example*)ex;
    char *line=NULL;
    size_t num_chars_initial = 0;
    if(all->daemon&&all->compress_predict)
    {
        ex->end_packet = false;
        num_chars_initial = readto_packet(*(all->p->input), line, '\n');
        //std::string s(line, num_chars_initial);
        //cerr<<num_chars_initial<<':'<<s.c_str()<<endl;
        if(num_chars_initial == PACK_SIZE&& strncmp(line, PACK_END, PACK_SIZE) == 0)
        {
            cerr<<"got end of the packet"<<endl;
            ex->end_packet = true;
            //return 1;
        }
    }
    else{
        num_chars_initial = readto(*(all->p->input), line, '\n');
    }
