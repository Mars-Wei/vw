...
int main(int argc, char* argv[]) {
    ...
    while(true) {
        ...
        {//Time to make the spanning tree
            cerr<<"time to make the spanning tree"<<endl;
            ...
            for(size_t i = 0;i < total;i++) {
                int done = 0;
                //recv node's port
                done = recv(partial_nodeset.nodes[i].socket, (char*)&(client_ports[i]), sizeof(client_ports[i]), 0); 
                if (done < (int)sizeof(client_ports[i]) )
                    cerr<<" Port read failed for node "<<i<<" read "<<done<<endl;
                else
                    cerr<<" Port read ok for node "<<i<<endl;
            ...
            for (size_t i = 0; i < total; i++)
            {
                if (parent[i] >= 0)
                {
                    cerr<<"send its parent:"<<parent[i]<<" 's ip to node "<<i<<endl;
                    ...
                    cerr<<"send its parent:"<<parent[i]<<" 's port to node "<<i<<endl;
